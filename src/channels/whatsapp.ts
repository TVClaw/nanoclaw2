...
const mentionTrigger = getTriggerPattern();
    ...
    this.sock.ev.on('connection.update', (update) => {
      ...
      if (connection === 'close') {
        ...
        if (shouldReconnect) {
          ...
          logger.info('Logged out. Re-run npm run auth or /setup.');
          process.exit(0);
        }
        ...
        try {
          ...
          if (groups[chatJid]) {
            ...
            if (mentioned && mentioned.length > 0 && this.sock.user?.id) {
              ...
              if (mentionsAssistant && !mentionTrigger.test(content.trim())) {
                content = `${DEFAULT_TRIGGER} ${content}`;
              }
            }

...
**Games on the brain** (same LAN): open with **OPEN_URL** only for files that exist under `games/` on the project (see group CLAUDE.md for the shipped list). If the user wants a game that is not shipped, use `<vibe-page>` with `<!-- nanoclaw:phone-remote -->` and a full inline HTML game: **inside the iframe**, `new EventSource('/vibe-key-sse')` (path-only; never `location.host` for the SSE URL), keypad command handling, `canvas.focus()` — see **`/vibe-page`** skill. Not a bogus `/games/...` URL. **Do not** tell the user to open `.../keypad` or paste the brain keypad URL in chat or `send_message`. For built-in brain games or phone-remote vibe HTML, the TV shows a QR — then say scan QR and **START**. For plain **vibe** pages without `<!-- nanoclaw:phone-remote -->`, the page opens directly; do not tell them to scan a gamepad QR unless you know that gate is active.
